import { DailyLog, SummaryStats, TrendInsights, TrendPoint } from "../models/types.js";
import {
  endOfMonth,
  endOfWeek,
  listDatesInRange,
...
export class SummaryService {
  getDailySummary(log: DailyLog | null): SummaryStats {
    if (!log) {
      return {
        totalMinutes: 0,
        totalExercises: 0,
  ...
  getWeeklySummary(logs: DailyLog[], dateString: string): SummaryStats {
  ...
  getMonthlySummary(logs: DailyLog[], dateString: string): SummaryStats {
    const date = parseDate(dateString);
  ...
  getDailyTotalsForWeek(logs: DailyLog[], dateString: string): TrendPoint[] {
    const date = parseDate(dateString);
    const start = startOfWeek(date);
    const end = endOfWeek(date);
  ...
  getDailyTotalsForMonth(logs: DailyLog[], dateString: string): TrendPoint[] {
    const date = parseDate(dateString);
    const start = startOfMonth(date);
    const end = endOfMonth(date);
  ...
  getTrendSeries(logs: DailyLog[]): TrendPoint[] {
  ...
  getInsights(logs: DailyLog[]): TrendInsights {
  ...
  private createSummary(logs: DailyLog[]): SummaryStats {
    const breakdownByExercise: Record<string, number> = {};
    let totalMinutes = 0;
  ...
  private sumLogMinutes(log: DailyLog): number {
    return log.exercises.reduce((sum, exercise) => sum + exercise.durationMinutes, 0);
  }

  private mapDailyTotals(logs: DailyLog[]): Map<string, number> {
  ...
  private calculateWeeklyGrowth(logs: DailyLog[]): number {
    if (logs.length === 0) {
      return 0;
    }
  ...
  private sumWithinRange(logs: DailyLog[], start: Date, end: Date): number {
    let total = 0;
    for (const log of logs) {
      const logDate = parseDate(log.date);
  ...
  private getWeeklyTotals(logs: DailyLog[]): number[] {
